...
export type Theme = {
  label: string;
  tokens: ThemeTokens;
  /** Optional per-theme font overrides (CSS font-family stacks). */
  fonts?: {
    serif?: string;
    sans?: string;
  };
...
export const themes = {
  ...
  /**
   * Ajrak — Sindhi heritage. Deep maroon ground with an Ajrak block-print
   * feel: crimson panels, indigo features, warm-ivory text, antique-gold accents.
   *   Maroon  #6B0F0F   Crimson #8B1212   Indigo #1B233A
   *   Ivory   #F5EBDD   Gold    #D4B06A
   * The card background is RED — text is ivory, accents are gold.
   */
  ajrak: {
    label: "Ajrak",
    tokens: {
      paper: "#6B0F0F", // card ground — deep Ajrak maroon
      paperSoft: "#8B1212", // tiles / sheets — rich crimson
      ...
      shadow: "0 28px 90px -20px rgba(0,0,0,0.8)",
    },
    fonts: {
...
export function themeStyle(name: ThemeName): React.CSSProperties {
  ...
  // Per-theme font overrides (fall back to the global defaults in globals.css).
